...
Scenari:
  A) Standard StratifiedKFold (5-fold). Every fold sees all tissues.
     Reproduces the train_task1.py setting but in CV form → more stable
     estimate (mean +- std) than a single train/test split.

  B) GroupKFold per tessuto (5-fold). I tessuti nel test NON sono nel train.
     Testa la vera generalizzazione a tessuti mai visti.
     If AUC drops -> the model was using tissue as a proxy for p53.

  C) Tissue-only baseline. Features: only tissue one-hot, no genes.
...
in the strictest case (LeaveOneGroupOut).
"""
import warnings
...
def scenario_A_standard_cv(X, y):
    ...
    print("una CV 'normale'. Number atteso ~ single-split AUC.")
...
def main():
    ...
    print("\nDistribution TP53 per tessuto (top 15):")
    ...
    compare.to_csv(OUTPUT_DIR / "task1/cv_comparison.csv", index=False)
    ...
    per_tissue.to_csv(OUTPUT_DIR / "task1/per_tissue_auc.csv", index=False)

    print("\nFile salvati:")
    print(f"  - {OUTPUT_DIR / 'task1/cv_comparison.csv'}")
    print(f"  - {OUTPUT_DIR / 'task1/per_tissue_auc.csv'}")
