...
def build_features_and_target(df: pd.DataFrame):
    if TARGET_COL not in df.columns:
        raise ValueError(f"Colonna target non trovata: {TARGET_COL}")
...
def main():
    ...
    print(f"\nNumero feature usate: {len(feature_cols)}")
    print(f"Esempio prime 10 feature: {feature_cols[:10]}")
    ...
    results_df.to_csv(output_dir / "task1_model_comparison.csv", index=False)

    print("\nRisultati salvati in:")
    print(output_dir / "task1_model_comparison.csv")
    ...
    if "Random Forest" in fitted_models:
        ...
        rf_importances.to_csv(output_dir / "task1_random_forest_feature_importance.csv", index=False)

        print("\nTop 20 feature - Random Forest:")
    ...
    if "XGBoost" in fitted_models:
        ...
        xgb_importances.to_csv(output_dir / "task1_xgboost_feature_importance.csv", index=False)

        print("\nTop 20 feature - XGBoost:")
